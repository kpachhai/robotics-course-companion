...
Starter. Three pieces are left for you, marked TODO(you): the action
normaliser, the CVAE's latent, and the confidence interval. Nothing runs until
all three are in.
...
class Norm:
    def __init__(self, x, floor=1e-3):
        # TODO(you): per-dimension mean and standard deviation of x.
        #            Floor the standard deviation: a dimension that never varies
        #            would otherwise divide by zero and produce NaN everywhere.
        raise NotImplementedError

    def to(self, x):
        # TODO(you): numpy in, float32 numpy out, zero mean and unit spread.
        raise NotImplementedError

    def back(self, x):
        # TODO(you): torch tensor in, torch tensor out, back in real units.
        raise NotImplementedError
...
class Cvae(nn.Module):
    ...
    def forward(self, obs, chunk):
        ...
        # TODO(you): draw z = mu + std * standard normal noise, with
        #            std = exp(0.5 * logvar). Sampling rather than using mu is
        #            what stops the latent becoming a lookup key for the
        #            training set.
        # TODO(you): the KL term that pulls the posterior towards a standard
        #            normal: -0.5 * sum(1 + logvar - mu^2 - exp(logvar)),
        #            summed over the latent dimensions, averaged over the batch.
        # Return (the decoder's output for this z, the KL term).
        raise NotImplementedError
...
def wilson(successes, n, z=1.96):
    ...
    # TODO(you): the Wilson score interval. With p = successes / n and
    #            d = 1 + z^2/n:
    #              centre = (p + z^2 / (2n)) / d
    #              half   = z * sqrt(p(1-p)/n + z^2/(4n^2)) / d
    #            Return (centre - half, centre + half), clipped to [0, 1].
    #            Sanity check: 8 successes out of 10 should give roughly
    #            0.49 to 0.94. That width is the point.
    raise NotImplementedError
