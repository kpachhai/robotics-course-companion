...
Starter. The two halves of diffusion - the forward process and the sampling
loop - are left for you, marked TODO(you). Everything else is done.
...
def q_sample(a0, k, eps):
    ...
    """
    _, _, abar = schedule()
    # TODO(you): return sqrt(abar[k]) * a0 + sqrt(1 - abar[k]) * eps.
    #            abar[k] is a 1D tensor of per-sample values, so it needs an
    #            unsqueeze(-1) before it multiplies an (N, 2) block.
...
def sample(net, obs, seed=SEED, keep=(), steps=K):
    ...
    a = torch.randn((len(ob), 2), generator=g)          # start from pure noise
    ...
    for i, k in reversed(list(enumerate(ks))):
        ...
        eps = net(a, kk, ob)
        # TODO(you): the full-length case first, and get that working before
        #            touching the rest:
        #              a <- (a - betas[k] / sqrt(1 - abar[k]) * eps) / sqrt(alphas[k])
        #            then, if k > 0, add betas[k].sqrt() * fresh standard noise.
        #
        # TODO(you): the skipped-step case (len(ks) < K) needs the deterministic
        #            update instead. Estimate the clean action
        #              a0 = (a - sqrt(1 - abar[k]) * eps) / sqrt(abar[k])
        #            then re-noise it to the PREVIOUS index in ks:
        #              a <- sqrt(prev) * a0 + sqrt(1 - prev) * eps
        #            with prev = abar[ks[i - 1]], or 1.0 when i == 0.
        raise NotImplementedError
    snaps[0] = a.numpy()
