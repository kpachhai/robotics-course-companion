"""Lesson 3.18 - one task, three artifacts, one scoreboard.

The groove world from lessons 3.4 and 3.5, solved three ways: by a rule you
wrote, by demonstrations you gave, and by a reward you scored. Same observation,
same action space, same success predicate. The only difference is what a human
had to author, and what that authoring cost.

Three TODOs, in the order the lesson needs them:

  1. `episode`   the success predicate. Every number below depends on it, so it
                 gets written once, before anything is trained.
  2. `reward`    the score the search optimises. This is the artifact reinforcement
                 learning asks you for, and writing it is the exercise.
  3. `search`    the elite update inside the cross-entropy method: five lines.

Run:  python three_ways.py            the scoreboard, about 100 s on a laptop CPU
...
def episode(policy, theta0, horizon=HORIZON):
    """Run one episode. Return (success, failure_tag_or_None, step_it_ended).

    TODO(you): success = the pin reaches B without ever leaving the groove, and
    the order matters. `gw.rollout` hands you `pins` and `devs`; find the first
    step where `devs` exceeds `gw.BAND`, find the first step within `gw.REACH` of
    `gw.B_TIP`, and count the run a success only if arrival came first. Tag the
    failures "left_groove" and "never_arrived" so the histogram is worth reading.
    """
...
def reward(vec, starts, horizon=HORIZON):
    """Score one candidate policy. Return (mean reward over starts, env steps used).

    TODO(you): roll out `make_net(vec)` from each start, and score it by how far
    along the groove it got BEFORE it first left, plus 1.0 for finishing. Use
    `path_progress` on the prefix of `pins` up to the first out-of-band step.

    Write your first attempt without reading further, run it, and look at what
    the search produces. Two rewards that seem obviously right are both gamed
    inside a minute: scoring the furthest path point ever approached buys a
    policy that cuts the corner straight to B, and scoring progress over in-band
    samples only buys one that leaves the groove, flies to B and lands back in
    band at the far end. Lesson 3.14 is about exactly this, and the fact that a
    task this simple has this many wrong rewards IS the finding.
    """
    raise NotImplementedError
...
def elite_update(pops, scores, elite):
    ...
    Return (order, mu, sigma) where `order` indexes the elite highest scores,
    best first.

    TODO(you): five lines. `mu` is the mean of the elite vectors, `sigma` their
    standard deviation plus a small floor (0.02) so the search cannot collapse to
    a point and stop exploring. Everything else in the cross-entropy method is
    already written.
    """
    raise NotImplementedError
