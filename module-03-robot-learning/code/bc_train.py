"""Lesson 3.3 - behaviour cloning: fit a network to the demonstrations.

Same dataset and same split as lesson 3.2, so the numbers land in the same table.
Four TODOs. When they are right you should beat every policy from the last
lesson, including the one that never moves.
...
def standardise(train, other):
    ...
    """
    # TODO(you): mean and std over axis 0 of `train`, guard the std against
    # zero with + 1e-6, and return both arrays scaled by the same numbers.
...
def build_net(n_in, n_out, hidden=HIDDEN):
    ...
    """
    # TODO(you): nn.Sequential of Linear(n_in, hidden), ReLU, Linear(hidden,
    # hidden), ReLU, Linear(hidden, n_out).
    raise NotImplementedError
...
def train_epoch(net, opt, X, Y, order):
    ...
    for i in range(0, len(order), BATCH):
        ...
        # TODO(you): forward pass, mean-squared error against Y[batch],
        # opt.zero_grad(), loss.backward(), opt.step(), and accumulate
        # loss.item() * len(batch) into `total`.
        raise NotImplementedError
...
def action_error(net, X_scaled, Y_raw, mean, std):
    ...
    """
    # TODO(you): run the net under torch.no_grad(), multiply by `std`, add
    # `mean`, and return the mean absolute difference from Y_raw.
    raise NotImplementedError
