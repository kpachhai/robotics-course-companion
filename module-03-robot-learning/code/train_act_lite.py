...
class ChunkDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, i):
        ...
        # TODO(you): take up to `self.chunk` actions starting at t. Near the end
        #            of an episode there are fewer, so repeat the last action to
        #            fill the window and build a float mask that is 1.0 on real
        #            steps and 0.0 on padded ones.
        # TODO(you): normalise the state and the action window with self.stats,
        #            which is (state_mean, state_std, action_mean, action_std).
        # Return three tensors: state (6,), window (chunk, 6), mask (chunk,).
...
def train(args):
    ...
    for epoch in range(args.epochs):
        ...
        for state, chunk, mask in loader:
            # TODO(you): forward, loss via act_loss(..., mask=mask), backward,
            #            step, zero_grad. Accumulate the RECONSTRUCTION term in
            #            `run`, not the total: the KL term is not an error the
            #            robot cares about, and mixing them makes the curve
            #            impossible to read.
            raise NotImplementedError
    ...
    Path(__file__).with_name(f"train_{args.model}_chunk{args.chunk}.json").write_text(
        json.dumps(out))
    return out
...
def evaluate(model, loader):
    ...
    Note what is NOT passed: the action chunk. Evaluation must run the
    inference path, or you are measuring a model that got to peek at the answer.
    """
    model.eval()
    total, count = 0.0, 0.0
    for state, chunk, mask in loader:
...
def per_chunk_index_error(model, loader):
    ...
    for state, chunk, mask in loader:
        pred, _, _ = model(state)
        per_step = (pred - chunk).abs().mean(dim=-1)
        s, c = (per_step * mask).sum(dim=0), mask.sum(dim=0)
