"""Lesson 3.5 - three ways to give the policy data where it actually goes.
...
Three TODOs: two in `dagger`, one in `collect_late`.
...
def collect_late(n_episodes, seed, offset=0.06):
    ...
    for k in range(n_episodes):
        ...
        for t in range(DEMO_STEPS):
            ...
            theta = theta + action
            # TODO(you): when t == shove_at, push the arm sideways by
            # sideways(theta, offset * rng.uniform(0.3, 1.0), rng).
...
def dagger(seed, seed_demos=20, rounds=5, per_round=3, rollout_len=120,
    ...
    for round_index in range(rounds + 1):
        ...
        # TODO(you): roll `policy` out `per_round` times for `rollout_len` steps
        # each, with walls=False so a crash does not cut the rollout short, and
        # collect every state it visits into one array called `visited`.
        raise NotImplementedError

        # TODO(you): ask the expert what it would have done at each of those
        # states, then append `visited` and those labels to `states`/`actions`.
        # This is the whole algorithm. The states are the policy's; the actions
        # are the expert's.
        raise NotImplementedError
