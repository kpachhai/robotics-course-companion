"""Solution - Lesson 3.13: the evaluation harness.
...
def paired_schedule(protocol: Protocol):
    ...
    """
    order = list(protocol.policies)
    out = []
    for i in range(protocol.n_trials):
        shift = i % len(order)
        for policy in order[shift:] + order[:shift]:
            out.append((policy, i, protocol.seed0 + i))
    return out
...
def summarise(trials):
    ...
    out = {}
    for policy in sorted({t.policy for t in trials}):
        mine = [t for t in trials if t.policy == policy]
        k, n = sum(t.success for t in mine), len(mine)
        lo, hi = wilson_interval(k, n)
        out[policy] = {
            "k": k, "n": n, "rate": k / n, "lo": lo, "hi": hi,
            "failures": Counter(t.failure for t in mine if not t.success),
        }
    return out
...
def compare(trials, policy_a, policy_b):
    ...
    by_index = {}
    for t in trials:
        by_index.setdefault(t.index, {})[t.policy] = t.success

    cells = Counter()
    for outcomes in by_index.values():
        if policy_a not in outcomes or policy_b not in outcomes:
            continue
        cells[(outcomes[policy_a], outcomes[policy_b])] += 1

    a_only, b_only = cells[(True, False)], cells[(False, True)]
    discordant = a_only + b_only
    p = binomtest(a_only, discordant, 0.5).pvalue if discordant else 1.0
    return {"both": cells[(True, True)], "neither": cells[(False, False)],
            "a_only": a_only, "b_only": b_only, "n_pairs": sum(cells.values()),
            "p_value": float(p)}
...
if __name__ == "__main__":
    ...
    print("solution eval_harness: protocol gate, pairing and rotation all check out.")
