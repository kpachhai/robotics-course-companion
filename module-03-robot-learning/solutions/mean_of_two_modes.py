...
def sample_arm(n, rng, elbow=None):
    ...
    t2 = t2 * (rng.choice([-1.0, 1.0], n) if elbow is None else elbow)
    theta = np.stack([t1, t2], axis=1)
    return theta, fk(theta)
...
def mode_histogram(target, theta, xy, radius=0.03, bins=36):
    ...
    """
    near = np.linalg.norm(xy - np.asarray(target), axis=1) < radius
    counts, edges = np.histogram(theta[near, 1], bins=bins, range=(-np.pi, np.pi))
    return counts, edges, int(near.sum())
...
def mixture_sweep(fractions=(1.0, 0.9, 0.75, 0.5), steps=800, n=20000):
    ...
    for frac in fractions:
        ...
        t1 = rng.uniform(-np.pi, np.pi, n)
        t2 = rng.uniform(0.35, np.pi - 0.35, n)
        t2 *= np.where(rng.random(n) < frac, 1.0, -1.0)      # frac of them elbow-up
        theta = np.stack([t1, t2], axis=1)
        xy = fk(theta)
        theta_v, xy_v = sample_arm(2000, np.random.default_rng(SEED + 22), elbow=+1)
        net, _ = fit(xy, as_circle(theta), steps=steps)
        miss = np.linalg.norm(fk(from_circle(predict(net, xy_v))) - xy_v, axis=1)
        rows.append((frac, float(np.median(miss))))
        print(f"  {frac:>5.0%} elbow-up   median miss {rows[-1][1]:.3f} m")
    return rows
