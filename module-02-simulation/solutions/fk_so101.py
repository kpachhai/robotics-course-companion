"""Solution - Lesson 2.9: your own forward kinematics for the SO-101, checked
against MuJoCo's.

The maths here is entirely Module 1: quaternion to rotation matrix, homogeneous
transforms, chain composition. MuJoCo supplies only the link geometry (the fixed
pos/quat of each body), which is what a model file is for.
...
def fk(q, offsets, site_offset):
    ...
    for k, offset in enumerate(offsets):
        pose = pose @ offset
        if k > 0:                       # 'base' is welded to the world
            pose = pose @ T(Rz(q[k - 1]), np.zeros(3))
    return pose @ site_offset
