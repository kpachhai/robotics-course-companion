"""Solution - Lesson 2.9: your Module 1 damped-least-squares IK, driving the
SO-101, and the point where agreement with the physics stops.

Imports the forward kinematics you wrote in fk_so101.py, so finish that first.
...
def jacobian_fd(q, offsets, site_offset, eps=1e-6):
    ...
    for j in range(5):
        nudged = q.copy()
        nudged[j] += eps
        cols.append((fk(nudged, offsets, site_offset)[:3, 3] - here) / eps)
...
def ik_step(target, q, offsets, site_offset, lo, hi, lam=LAMBDA, alpha=ALPHA):
    ...
    return np.clip(q + alpha * dq, lo, hi), float(np.linalg.norm(error))
