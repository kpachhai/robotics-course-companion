"""Lesson 2.13 - configuration space of the 2-link planar arm (reference solution).
...
def collides(q, obstacles=BENCH):
    ...
    for ox, oy, orad in obstacles:
        ...
        clearance = orad + LINK_RADIUS
        if segment_point_distance(base, elbow, centre) < clearance:
            return True
        if segment_point_distance(elbow, tip, centre) < clearance:
            return True
    return False
...
def interpolate(q0, q1, steps):
    ...
    delta = (q1 - q0 + np.pi) % TAU - np.pi
    return [q0 + delta * s for s in np.linspace(0.0, 1.0, steps)]
...
def plot(n=360):
    ...
    # module-*/solutions/*.png, so running this from the repo root must not
