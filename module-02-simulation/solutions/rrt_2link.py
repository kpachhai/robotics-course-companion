"""Lesson 2.14 - sampling-based planning for the 2-link arm (reference solution).
...
def edge_free(a, b, obstacles=BENCH, resolution=EDGE_RES):
    ...
    """
    delta = wrap(np.asarray(b) - np.asarray(a))
    steps = max(2, int(np.ceil(np.linalg.norm(delta) / resolution)) + 1)
    checks = 0
    for s in np.linspace(0.0, 1.0, steps):
        checks += 1
        if collides(a + delta * s, obstacles):
            return False, checks
    return True, checks
...
def rrt(start, goal, obstacles=BENCH, seed=0, max_nodes=MAX_NODES,
    ...
    while len(nodes) < max_nodes:
        ...
        near = int(np.argmin(np.linalg.norm(wrap(target - nodes), axis=1)))
        delta = wrap(target - nodes[near])
        span = float(np.linalg.norm(delta))
        if span == 0.0:
            continue
        new = nodes[near] + delta * min(1.0, step / span)
...
def shortcut(path, obstacles=BENCH, iterations=200, seed=0, resolution=EDGE_RES):
    ...
    for _ in range(iterations):
        ...
        if j - i < 2:
            continue
        free, _ = edge_free(path[i], path[j], obstacles, resolution)
        if free:
