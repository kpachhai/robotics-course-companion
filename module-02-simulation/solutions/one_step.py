"""Solution - Lesson 2.2: one physics step, written out by hand.

Self-contained. Run:  python one_step.py
"""
import numpy as np
...
def spring(x0=1.0, v0=0.0, k=100.0, m=1.0, dt=0.01, steps=2000, semi_implicit=True):
    ...
    for _ in range(steps):
        a = -k * x / m
        v_new = v + a * dt
        v_used = v_new if semi_implicit else v      # the entire difference
        x = x + v_used * dt
        v = v_new
...
def drop_solved(dt, z0=0.30, m=1.0, sim_time=3.0, beta=0.2, softness=0.5):
    ...
    for _ in range(steps):
        ...
        if z + v_free * dt < 0.0:                    # this step would end illegal
            depth = max(0.0, -z)
            v_target = beta * depth / dt             # allowed separation speed
            lam = max(0.0, m * (v_target - v_free) / (1.0 + softness))
            v = v_free + lam / m
        else:
            v = v_free
        z = z + v * dt
...
def stack_solved(sweeps, dt=0.002, sim_time=2.0, r=0.05, m=1.0, beta=0.2, softness=0.5):
    ...
    `sweeps` is how many passes the solver makes over the two rows per step.
...
def main():
    ...
    e0, e_exp = spring(semi_implicit=False)
    _, e_semi = spring(semi_implicit=True)
    assert e_exp / e0 > 100, "explicit Euler should pump energy into the spring"
    assert 0.5 < e_semi / e0 < 2.0, "semi-implicit should stay near the start energy"
    assert drop_penalty(1e7)[1], "a stiff enough penalty spring must blow up at this dt"
    assert not drop_solved(0.05)[1], "the solved contact must survive a 50 ms timestep"
    assert stack_solved(1) > 3 * stack_solved(20), "one sweep must not settle the stack"
    print("\n✅ solution one_step: integrate, find what is illegal, solve, commit.")
