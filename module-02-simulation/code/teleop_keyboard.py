"""Lesson 2.10 - keyboard teleoperation of the SO-101 in MuJoCo.
...
class Teleop:
    ...
    def on_key(self, keycode):
        ...
        # TODO(you): if key is in JOG, add that offset to self.target.
        #            If it is in GRIP, add that delta to self.grip and clip it
        #            into GRIP_RANGE. If it is "R", go back to self.home.
        #            Anything else: ignore it, quietly.
        raise NotImplementedError

    def control(self):
        """One IK update, warm-started from the last command, then write ctrl."""
        # TODO(you): call ik_step(self.target, self.q_cmd, ...) and keep the new
        #            q_cmd. Write it into self.data.ctrl[:5], and self.grip into
        #            self.data.ctrl[5]. Do NOT reset q_cmd between frames: the
        #            warm start is what makes one step per frame enough.
        raise NotImplementedError
