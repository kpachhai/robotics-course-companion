    ...
    for i, y in enumerate(ys):
        for j, x in enumerate(xs):
            if np.hypot(x, y) < 0.05:
                continue
            # TODO(you): call sp.ik() for the pose (x, y, z) starting from
            # arm.data.ctrl[:5], and mark the cell reachable when both the
            # position residual and the rotation residual are inside tolerance.
...
def carry_ceiling(arm, radius=0.20, pos_tol=2e-3, rot_tol=0.05):
    ...
    best = 0.0
    # TODO(you): walk z upward and keep the highest value the IK still solves.
    raise NotImplementedError
