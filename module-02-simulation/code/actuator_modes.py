"""Lesson 2.7 - MuJoCo actuator types on a one-joint bench.

The model is complete. Two functions are yours.

Run:  python actuator_modes.py          # all three parts
      python actuator_modes.py b        # just the PD identity
"""
import sys
...
# Same joint, same bob, same gravity. Only this line changes between the runs.
...
def run(model, ctrl, seconds):
    ...
    TODO(you):
      1. data = mujoco.MjData(model)
      2. write `ctrl` into data.ctrl[0] once - MuJoCo never clears it
      3. call mujoco.mj_step(model, data) for seconds / model.opt.timestep steps
      4. return data
    """
...
def pd_force(target, q, qd):
    ...
    TODO(you): return kp*(target - q) - kv*qd, clipped to +/- FMAX.
    Same three symbols as your PID from lesson 1.17, minus the integral term.
    """
    raise NotImplementedError
